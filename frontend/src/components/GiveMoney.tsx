...
export function GiveMoney({
	...
	const program = useMemo(
		() =>
			new Program<TamashiiSolanaMiningPool>(
	...
	const tamashiiPoolPda = useMemo(
		() =>
			getMiningPoolPDA(new PublicKey(miningPoolProgramId), TAMASHII_POOL_INDEX),
		[miningPoolProgramId]
	)
		...
		<>
			...
			<Wrapper>
				{sending.type === 'sent' ? (
					...
					<>
						...
						<Balance className={text['body/sm/medium']}>
							...
							for future updates about tamashii
						</Balance>
					</>
				) : (
					<>
				...
				<span className={text['aux/xs/regular']}>
					...
					the Tamashii Foundation on the Testnet, any tokens configured using the
